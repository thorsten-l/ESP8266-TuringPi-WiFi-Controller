...
void WebHandler::setup()
{
  ...
  if ( appcfg.wifi_mode == WIFI_AP )
  {
    server.on("/captive.html", HTTP_GET, []() {
      server.send(200, "text/html", 
        F("Please enter <b><a target='_blank' href='http://turing.pi'>http://turing.pi</a></b> or <b><a target='_blank' href='http://192.168.192.1'>http://192.168.192.1</a></b> into your browser."));
      server.client().stop();
    });
  }
  ...
  server.onNotFound([]{
    if ( appcfg.wifi_mode == WIFI_AP )
    {
      server.sendHeader( "Location", "http://turing.pi/captive.html", true );
